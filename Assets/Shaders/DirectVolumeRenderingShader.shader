Shader "VolumeRendering/DirectVolumeRenderingShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct frag_out { float4 colour : SV_TARGET;
#if DEPTHWRITE_ON
            ...
            struct RayInfo {  float3 startPos; float3 endPos; float3 direction; float2 aabbInters; };

            struct RaymarchInfo { RayInfo ray;  int numSteps; float numStepsRecip; float stepSize; };

            float3 getViewRayDir(float3 vertexLocal)  {
                if (unity_OrthoParams.w == 0)  {    
                    return normalize(ObjSpaceViewDir(float4(vertexLocal, 0.0f))); // Perspective
                }
                else {
                    // Orthographic
            ...
            float4 getTF1DColour(float density) {  return tex2Dlod(_TFTex, float4(density, 0.0f, 0.0f, 0.0f)); }

            // Gets the colour from a 2D Transfer Function (x = density, y = gradient magnitude)
            float4 getTF2DColour(float density, float gradientMagnitude) {  return tex2Dlod(_TFTex, float4(density, gradientMagnitude, 0.0f, 0.0f)); }

            // Gets the density at the specified position
            float getDensity(float3 pos) {  return tex3Dlod(_DataTex, float4(pos.x, pos.y, pos.z, 0.0f)); }

            // Gets the gradient at the specified position
            float3 getGradient(float3 pos) {  return tex3Dlod(_GradientTex, float4(pos.x, pos.y, pos.z, 0.0f)).rgb; }
            ...
            {
                float ndotl = max(lerp(0.0, _LightIntensity, dot(normal, lightDir)), 0.0); // modified, to avoid volume becoming too dark
                float3 diffuse = ndotl * col;
                float3 v = eyeDir;
                float3 r = normalize(reflect(-lightDir, normal));
                float rdotv = max( dot( r, v ), 0.0 );
                float3 specular = pow(rdotv, 32.0f) /* * float3(1.0f, 1.0f, 1.0f) doesnt make any visual changes -Alex */ * specularIntensity;
